...
const POOL = {
  ...
  /**
   * ⚠️ لا تُضِف `allowExitOnIdle: true` هنا. جُرِّب فأسقط المتجر بالكامل:
   * المسبح يُنهي نفسه عند الفراغ بينما عميل Prisma ما زال ممسكًا به،
   * فيفشل **كل** استعلام تالٍ لا الطلبات المتزامنة وحدها. القياس: 500 على
   * طلب واحد منفرد بعد دقيقة سكون.
   */
} as const;
